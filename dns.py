...
class SimpleDNSProtocol(protocol.DatagramProtocol):
    ...
    def logQuery(self, query, address):
        log.msg(
            f"DNS Query Received - Query Name: {query.name.name}, Type: {dns.QUERY_TYPES.get(query.type, 'UNKNOWN')}, Class: {dns.QUERY_CLASSES.get(query.cls, 'UNKNOWN')}, From: {address}"
        )
    ...
    def createResponse(self, message, query):
        ...
        query_name_str = (
            ...
            if isinstance(query.name.name, bytes)
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=5353, help="Port to bind the server to."
    )
